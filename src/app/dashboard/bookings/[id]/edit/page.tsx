...
export default function EditBookingPage() {
  ...
  const loadEditBooking = useCallback(
    async (refresh = false) => {
      if (!bookingId) {
      ...
      try {
        ...
        const [bookingResponse, accountResponse, driverResponse] =
          await Promise.all([
            api.get<BookingDetail>(`/bookings/${bookingId}`),
            api.get<ClientAccount[]>("/clients/accounts?active=true"),
        ...
        let loadedBookers: Booker[] = [];
        let loadedPassengers: Passenger[] = [];

        if (accountId) {
          const [bookerResponse, passengerResponse] = await Promise.all([
            api.get<Booker[]>(`/clients/bookers?accountId=${accountId}`),
            api.get<Passenger[]>(`/clients/passengers?accountId=${accountId}`),
          ]);

          loadedBookers = bookerResponse.data;
          loadedPassengers = passengerResponse.data;
        }

        const primaryBookerRelation =
          booking.bookers.find((item) => item.isPrimary) || booking.bookers[0];

        const primaryPassengerRelation =
          booking.passengersList.find((item) => item.isPrimary) ||
          booking.passengersList[0];

        const londonPickup = isoToLondonDateTimeFields(booking.pickupDatetime);
        ...
        setBookers(loadedBookers);
        setPassengers(loadedPassengers);

        hydratedAccountRef.current = accountId;
        setSelectedAccountId(accountId);

        setPrimaryBookerId(primaryBookerRelation?.bookerId || "");
        setAdditionalBookerIds(
          booking.bookers
            .filter((item) => item.bookerId !== primaryBookerRelation?.bookerId)
            .sort((a, b) => a.position - b.position)
            .map((item) => item.bookerId),
        );

        setPrimaryPassengerId(primaryPassengerRelation?.passengerId || "");
        setAdditionalPassengerIds(
          booking.passengersList
            .filter(
              (item) =>
                item.passengerId !== primaryPassengerRelation?.passengerId,
            )
            .sort((a, b) => a.position - b.position)
            .map((item) => item.passengerId),
        );

        setAccountFields({
          costCenter: booking.costCenter || "",
          invoiceRef: booking.invoiceRef || "",
          jobStatus: booking.jobStatus || "CONFIRMED",
          salesman: booking.salesman || "",
          nameCardOverride: booking.nameCardOverride || "",
          nameCardFileUrl: booking.nameCardFileUrl || "",
        });

        setJourneyFields({
          journeyType: booking.journeyType || "TRANSFER",
          pickupDate: londonPickup.date,
          pickupTime: londonPickup.time,
          pickupAddress: booking.pickupAddress || "",
          dropoffAddress: booking.dropoffAddress || "",
          passengerCount: String(booking.passengers ?? 1),
          bigLuggage: String(booking.bigLuggage ?? 0),
          smallLuggage: String(booking.smallLuggage ?? 0),
          babySeats: String(booking.babySeats ?? 0),
          childSeats: String(booking.childSeats ?? 0),
          boosterSeats: String(booking.boosterSeats ?? 0),
          estimatedDurationMinutes: String(
            booking.estimatedDurationMinutes ?? 120,
          ),
          routeDistanceMeters:
            booking.routeDistanceMeters === null
              ? ""
              : String(booking.routeDistanceMeters),
          routeDurationSeconds:
            booking.routeDurationSeconds === null
              ? ""
              : String(booking.routeDurationSeconds),
        });
        ...
        setAllocationFields({
          preferredVehicleCategory:
            booking.preferredVehicleCategory || "Executive",
          requestedDriverType: booking.requestedDriverType || "",
        ...
        setClientFinance({
          clientQuote: toInputValue(finance?.clientQuote),
          clientWaitingCharge: toInputValue(finance?.clientWaitingCharge),
          clientParking: toInputValue(finance?.clientParking),
          clientCongestion: toInputValue(finance?.clientCongestion),
          clientDiscount: toInputValue(finance?.clientDiscount),
          clientTotal: toInputValue(finance?.clientTotal),
          clientAdminFee: toInputValue(finance?.clientAdminFee),
          clientNet: toInputValue(finance?.clientNet),
          clientVat: toInputValue(finance?.clientVat),
          clientVatPercent: toInputValue(finance?.clientVatPercent, "20"),
          clientTotalAmount: toInputValue(finance?.clientTotalAmount),
          ...
          paymentType: finance?.paymentType || "ACCOUNT",
        });

        setDriverFinance({
          driverCost: toInputValue(finance?.driverCost),
          driverWaitingPay: toInputValue(finance?.driverWaitingPay),
          driverParking: toInputValue(finance?.driverParking),
          driverCongestion: toInputValue(finance?.driverCongestion),
          driverTotal: toInputValue(finance?.driverTotal),
  ...
  /* =====================================================
     LOAD BOOKERS + PASSENGERS WHEN ACCOUNT CHANGES
  ===================================================== */
  ...
  useEffect(() => {
    ...
    if (hydratedAccountRef.current === selectedAccountId) {
    ...
    loadPresets();
  }, [initialLoaded, selectedAccountId]);
  ...
  const canChangeDriver = ["PENDING", "ASSIGNED"].includes(bookingStatus);
  const isLocked = ["COMPLETED", "CANCELLED", "REJECTED"].includes(
    bookingStatus,
  );
  ...
  async function updateBooking() {
    ...
    if (!primaryPassenger.phone?.trim()) {
      setSubmitError(
        "The primary passenger must have a phone number before this booking can be updated.",
      );
      return;
    }
    ...
    [...new Set(additionalBookerIds)]
      .filter((id) => id && id !== primaryBookerId)
      .forEach((bookerId, index) => {
        bookingBookers.push({
          bookerId,
          isPrimary: false,
    ...
    const bookingVias = vias
      .map((via) => ({ address: via.address.trim() }))
    ...
    const payload: Record<string, unknown> = {
      ...
      customerEmail: primaryPassenger.email?.trim() || null,

      accountId: selectedAccountId,
      costCenter: accountFields.costCenter.trim(),
      invoiceRef: accountFields.invoiceRef.trim(),
      jobStatus: accountFields.jobStatus || "CONFIRMED",
      salesman: accountFields.salesman.trim(),
      nameCardOverride: accountFields.nameCardOverride.trim(),
      nameCardFileUrl: accountFields.nameCardFileUrl.trim(),
      ...
      routeDistanceMeters: optionalIntegerOrNull(
        journeyFields.routeDistanceMeters,
      ),
      routeDurationSeconds: optionalIntegerOrNull(
        journeyFields.routeDurationSeconds,
      ),
      estimatedDurationMinutes: estimatedDuration,

      preferredVehicleCategory: allocationFields.preferredVehicleCategory || "",
      requestedDriverType: allocationFields.requestedDriverType || "",

      notes: noteFields.notes.trim(),
      driverNote: noteFields.driverNote.trim(),

      bookers: bookingBookers,
      passengersList: bookingPassengers,
      vias: bookingVias,

      finance: {
        clientQuote: optionalMoneyOrNull(clientFinance.clientQuote),
        clientWaitingCharge: optionalMoneyOrNull(
          clientFinance.clientWaitingCharge,
        ),
        clientParking: optionalMoneyOrNull(clientFinance.clientParking),
        clientCongestion: optionalMoneyOrNull(clientFinance.clientCongestion),
        clientDiscount: optionalMoneyOrNull(clientFinance.clientDiscount),
        clientTotal: optionalMoneyOrNull(clientFinance.clientTotal),
        clientAdminFee: optionalMoneyOrNull(clientFinance.clientAdminFee),
        clientNet: optionalMoneyOrNull(clientFinance.clientNet),
        clientVat: optionalMoneyOrNull(clientFinance.clientVat),
        clientVatPercent: optionalMoneyOrNull(clientFinance.clientVatPercent),
        clientTotalAmount: optionalMoneyOrNull(clientFinance.clientTotalAmount),
        ...
        driverCost: optionalMoneyOrNull(driverFinance.driverCost),
        driverWaitingPay: optionalMoneyOrNull(driverFinance.driverWaitingPay),
        driverParking: optionalMoneyOrNull(driverFinance.driverParking),
        driverCongestion: optionalMoneyOrNull(driverFinance.driverCongestion),
        driverTotal: optionalMoneyOrNull(driverFinance.driverTotal),
        ...
        paymentType: clientFinance.paymentType || "",
        financeNote: financeNotes.financeNote.trim(),
        invoiceNote: financeNotes.invoiceNote.trim(),
        extraNote: financeNotes.extraNote.trim(),
      },
    };

    if (canChangeDriver) {
      payload.assignedDriverId = allocationFields.assignedDriverId || null;
    }

    try {
      setSavingBooking(true);

      const response = await api.patch<BookingDetail>(
        `/bookings/${bookingId}`,
        payload,
      );
      ...
      setSubmitSuccess(
        `Booking ${response.data.bookingReference} updated successfully.`,
      );
    ...
    <div className="space-y-6 p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Update the existing chauffeur booking and allocation details.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          onClick={() => {
            if (
              window.confirm(
                "Reload this booking? Unsaved changes will be lost.",
              )
            ) {
              loadEditBooking(true);
            }
          }}
          disabled={refreshing}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="inline-flex w-fit rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-700">
        ...
        {isLocked && (
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            This booking is {bookingStatus.toLowerCase()} and can no longer be
            edited.
          </div>
        )}
      </section>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <Field label="Assign Driver">
              <select
                ...
                className="booking-input"
                disabled={driverLoading || !canChangeDriver}
              >
                <option value="">
              ...
              <p className="mt-2 text-xs text-slate-400">
                {canChangeDriver
                  ? "Busy and offline drivers remain selectable for future bookings. The backend will reject actual time conflicts."
                  : "Driver assignment is locked after the booking has moved beyond Pending or Assigned."}
              </p>
            </Field>
          </div>
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
                Review the information above before saving your changes.
                {canChangeDriver
                  ? " Driver schedule conflicts will be checked before the update is accepted."
                  : " Driver assignment is locked at the current booking status."}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                disabled={savingBooking || isLocked}
...
function optionalIntegerOrNull(value: string): number | null {
  if (!value.trim()) {
    return null;
  }
...
function optionalMoneyOrNull(value: string): number | null {
  if (!value.trim()) {
    return null;
  }
...
function isoToLondonDateTimeFields(value: string) {
